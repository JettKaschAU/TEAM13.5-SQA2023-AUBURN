...
logging.basicConfig(filename='forensics_log.txt', level=logging.INFO) 
...
def getCountFromAnalysis(ls_):
    ...
    for tup_ in ls_:
        try: 
            ...
            logging.info(f"Analysis for script {script_name} completed successfully.")

        except Exception as e:
            logging.error(f"Error during analysis of script {script_name}: {str(e)}")
...
def main(directory: Path = typer.Argument(..., exists=True, help="Absolute path to the folder than contains Kubernetes manifests")):
    ...
    try:
        ...
        logging.info(f"Scan completed successfully. Results saved to: {outfile}")
